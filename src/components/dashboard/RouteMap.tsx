...
const MapboxTokenInput = ({
  ...
  return (
    <div className="p-4 border rounded-md bg-muted/20 mb-4" data-oid="dfjf3fr">
      <h3 className="font-semibold mb-2" data-oid="p21imy8">
        Mapbox Token Required
      </h3>
      <p className="text-sm text-muted-foreground mb-2" data-oid="29olnbh">
        Please enter your Mapbox public token to display the map. You can get
        one from your{" "}
        <a
      ...
      <div className="flex gap-2" data-oid="vzmm8nh">
        <input
          type="text"
          value={token}
...
export const RouteMap: React.FC<RouteMapProps> = ({ className }) => {
  ...
  const defaultToken =
    "pk.eyJ1IjoiZW1vbnNoYWhyaWVyIiwiYSI6ImNtODJnZjFzejBjbDQyam9ncTZ3cXhod20ifQ.M-GWDvnQh6ABad5o4eyM5Q";
    ...
    map.current.on("load", () => {
      ...
      for (let i = 0; i < journeyPoints.length - 1; i++) {
        ...
        const segmentDistance =
          nextPoint.distanceFromStart - currentPoint.distanceFromStart;
        ...
        if (accumulatedDistance + segmentDistance > totalDistance) {
          ...
          const nextCoord = [
            nextPoint.coordinates.lng,
            nextPoint.coordinates.lat,
          ];

          const interpolatedLng =
            currentCoord[0] + (nextCoord[0] - currentCoord[0]) * progress;
          const interpolatedLat =
            currentCoord[1] + (nextCoord[1] - currentCoord[1]) * progress;
      ...
      if (accumulatedDistance < totalDistance) {
        completedPoints.push([
          journeyPoints[journeyPoints.length - 1].coordinates.lng,
      ...
      new mapboxgl.Marker({ color: "#0ea5e9" })
        ...
        .setPopup(
          new mapboxgl.Popup().setHTML(
            "<strong>Boston</strong><br>Starting point",
          ),
        )
        .addTo(map.current);
      ...
      new mapboxgl.Marker({ color: "#f97316" })
        ...
        .setPopup(
          new mapboxgl.Popup().setHTML(
            "<strong>Rotterdam</strong><br>Final destination",
          ),
        )
        .addTo(map.current);
  ...
  return (
    <div className={className} data-oid="l-l.9qd">
      {!mapboxToken ? (
        <MapboxTokenInput onTokenSubmit={initializeMap} data-oid="s4xsk6d" />
      ) : null}
      <div
        ref={mapContainer}
      ...
      {mapboxToken && (
        <div className="flex justify-end mt-2" data-oid="j:h4j8y">
          <button
            onClick={() => {
